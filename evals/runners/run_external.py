...
async def run_external(
    *, dataset: str, path: str | Path, system_name: str, config_path: str | None = None,
    output: str | Path | None = None, limit: int | None = None,
    ...
    for example in examples:
        ...
        await repository.create_scope(
            ScopeCreate(
                id=scope_id, name=f"{dataset}:{example.example_id}", scope_type=ScopeType.CUSTOM,
        ...
        if system_name == "vector_memory":
            ...
            system = FlatGraphMemory(repository, extractor, embedder, retrieval_config=retrieval)
        ...
        for session in inputs:
            await system.ingest_session(
                ...
                current_scope=ScopeRef(
                    id=scope_id, name=f"{dataset}:{example.example_id}",
                    scope_type=ScopeType.CUSTOM, session_id=session.id,
                ),
            )
...
def main() -> None:
    ...
    print(asyncio.run(run_external(
        dataset=args.dataset, path=args.path, system_name=args.system,
        config_path=args.config, output=args.output, limit=args.limit,
